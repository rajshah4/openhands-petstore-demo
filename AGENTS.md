...
- Skills live under `.agents/skills/`; `code-review` and `releasenotes` are vendored from `OpenHands/extensions`, while the other demo flows remain custom repo-local skills.
...
- README should explain the mixed skill model: official `OpenHands/extensions` skills are vendored locally for `code-review` and `releasenotes`, while the other flows remain custom repo-local skills.
